...
export async function DELETE(
  ...
  const comment = await prisma.comment.findUnique({
    where: { id: params.id },
    include: { post: { select: { classId: true } }, poll: { select: { classId: true } } },
  });
  if (!comment) return NextResponse.json({ error: "Kommentar nicht gefunden." }, { status: 404 });

  const classId = comment.post?.classId ?? comment.poll?.classId;
  if (!classId) return NextResponse.json({ error: "Kommentar nicht gefunden." }, { status: 404 });
